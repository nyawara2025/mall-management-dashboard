...
import { VisitorWelcomePage } from './components/VisitorWelcomePage';
import { ScreenDisplay } from './components/ScreenDisplay';
import { PublicAgricHub } from './components/PublicAgricHub';
import { AgentDashboard } from './components/AgentDashboard';
import { ElectionCandidateModal } from './components/ElectionCandidateModal';
import { PublicLogisticsHub } from './components/PublicLogisticsHub';
import { DiocesanRouter } from './components/DiocesanRouter';
import { MediaPresentationController } from './components/MediaPresentationController';
...
function AppContent() {
  ...
  // =========================================================================
  // 🛡️ ZERO-DISRUPTION NATIVE APP URL RESTORER (Domain & Tenant Aware)
  // =========================================================================
  const isAndroidWebView = typeof navigator !== 'undefined' && /; wv\)/.test(navigator.userAgent);
  const currentUrl = new URL(window.location.href);
  const urlShopId = currentUrl.searchParams.get('shop_id');
  ...
  if (urlShopId) {
    ...
    localStorage.setItem('__native_shop_id', urlShopId);
    ...
    if (isSchoolDomain) {
      ...
      currentUrl.pathname = '/school';
      currentUrl.searchParams.set('business_category', cachedBizCat);
      currentUrl.searchParams.set('shop_id', cachedShopId);

    } else if (isAgriDomain || currentUrl.pathname === '/agri') {
      // 🚜 Agricultural Hub Fallbacks (New Intercept Entry)
      const cachedShopId = localStorage.getItem('__native_shop_id') || '81'; // Default to Nyawara Ranch
      const cachedBizCat = localStorage.getItem('__native_biz_cat') || 'agricultural';
      currentUrl.pathname = '/agri';
      currentUrl.searchParams.set('business_category', cachedBizCat);
      currentUrl.searchParams.set('shop_id', cachedShopId);

    } else if (isLogisticsDomain || currentUrl.pathname === '/logistics') {
      // 🚚 Commercial Transport Hub Fallbacks (New Intercept Entry)
      const cachedShopId = localStorage.getItem('__native_shop_id') || '90'; // Default Logistics Shop ID
      const cachedBizCat = localStorage.getItem('__native_biz_cat') || 'logistics';
      currentUrl.pathname = '/logistics';
      currentUrl.searchParams.set('business_category', cachedBizCat);
      currentUrl.searchParams.set('shop_id', cachedShopId); 

    } else {
      // ⛪ Church Hub Fallbacks (Your original logic)
      const cachedShopId = localStorage.getItem('__native_shop_id') || '68'; 
      const cachedBizCat = localStorage.getItem('__native_biz_cat') || 'church';

      currentUrl.pathname = '/church';
      currentUrl.searchParams.set('business_category', cachedBizCat);
      currentUrl.searchParams.set('shop_id', cachedShopId);
    }

    // Force an instant clean reload before React renders anything else
  ...
  const isAgriPath = window.location.pathname.includes('/agri');
  const isLogisticsPath = window.location.pathname.includes('/logistics') || window.location.pathname.startsWith('/truck'); // 🚚 Added for Logistics
  const isDiocesePath = window.location.pathname.includes('/diocese') || bizCategory === 'diocesan-erp';
  ...
  //  =========================================================================
  // 🔀 HIGH-PRIORITY REAL-TIME DISPLAY INTERCEPTOR
  // =========================================================================
  if (window.location.pathname === '/screen') {
    return <ScreenDisplay />;
  }

  if (window.location.pathname === '/broadcast-console') {
  ...
  // =========================================================================
  // 📱 DEDICATED HIGH-PRIORITY CANDIDATE MOBILE VIEW ROUTE INTERCEPT
  // =========================================================================
  if (window.location.pathname === '/candidate') {
    return <ElectionCandidateModal />;
  }
  ...
  if ((bizCategory === 'church' || isChurchPath) && !user) {
    ...
    const view = searchParams.get('view'); // Add this to detect the sub-page

    return (
      <div className="min-h-screen bg-gray-50">

        {/* If view=give is in URL, show the Giving Page, otherwise show Hub */}
        {view === 'give' ? (
          <PublicGivingPage /> 

        ) : view === 'welcome' ? (
          /* You can call a new VisitorWelcome component here */
          <VisitorWelcomePage shopId={Number(shopId) || 68} />
        ) : (
  ...
  // 🎓 DYNAMIC MULTI-ROLE EDUCATIONAL HUB GATEKEEPER
  if (bizCategory === 'educational' || isSchoolPath) {
    return (
      <div className="min-h-screen bg-gray-50">
        {/* 🌟 Swap PublicSchoolHub for SchoolRouter to support Director, Principal & Security panels */}
        <SchoolRouter 
          shopId={Number(shopId) || 69} 
          user={user} 
          onLogout={logout} 
        /> 
      </div>
  ...
  // 🚜 FARMER PUBLIC MOBILE HUB GATEKEEPER (New Intercept Injection)
  const isExplicitFarmPath = window.location.pathname.startsWith('/farm');

  if (bizCategory === 'agricultural' || isExplicitFarmPath) {
    return (
      <div className="min-h-screen bg-stone-50">
        {/* PublicAgriHub manages its internal login/registration state engine locally */}
        <PublicAgricHub />
      </div>
    );
  }
        ...
        <div className="min-h-screen bg-gray-50">
          ...
          {currentView === 'tithes' && (
            <div className="min-h-screen">
              <Header 
                onBack={() => setCurrentView('dashboard')} 
                title="Tithing & Financial Records" 
                user={user} 
                onLogout={logout} 
                showUserMenu={showUserMenu} 
                setShowUserMenu={setShowUserMenu} 
