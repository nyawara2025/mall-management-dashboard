...
export function CampaignHub() {
  ...
  const handleAction = async (actionType: string, sectionId: string, url?: string) => {
    // Log interaction for Analytics Dashboard
    fetch('https://n8n.tenear.com/webhook/political-campaign-interactions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (actionType === 'link' && url) {
      ...
      if (navigator.share) {
        navigator.share({ title: 'Campaign Hub', url: window.location.href });
      } else {
  ...
  const handleFeedbackSubmit = async (text: string, phone: string) => {
    const endpoint = modalMode === 'ai' 
      ? "https://n8n.tenear.com/webhook/political-AI" 
      : "https://n8n.tenear.com";
    ...
    try {
      await fetch(endpoint, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: modalMode === 'ai' ? 'query_rag' : 'submit_critique',
          shop_id: shopId,
          content: text,
          voter_phone: phone,
          business_category: 'political'
      ...
      alert(modalMode === 'ai' ? "AI is processing! Check WhatsApp soon." : "Team notified! Thank you.");
    } catch (e) {
