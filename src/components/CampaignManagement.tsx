 ...
 * - Example: "12-sandra-shop_admin-6-6-1763753837104"
...
type CampaignTemplateId = 'welcome-offer' | 'flash-sale' | 'gift-offer' | 'premium-spirits' | 'premium-grooming' | 'luxury-spa';
...
const CAMPAIGN_TEMPLATES: Record<CampaignTemplateId, CampaignTemplate> = {
  'welcome-offer': {
    ...
    icon: '👋',
    visitorMessage: {
      title: '👋 Welcome to {SHOP_NAME}!',
      body: 'Welcome to {SHOP_NAME}! We\'re excited to have you here. As our guest, enjoy exclusive visitor pricing and special offers just for you.',
      emoji: '👋',
      theme: {
        primaryColor: '#3B82F6',
  ...
  'premium-spirits': {
    ...
    description: 'Exclusive alcohol offers for premium customers',
    icon: '🥃',
    visitorMessage: {
      title: '🥃 Premium Spirits Collection',
      body: 'Discover our exclusive selection of premium spirits. As a special visitor, enjoy special pricing on our finest bottles.',
      emoji: '🥃',
      theme: {
        primaryColor: '#7C3AED',
        secondaryColor: '#F3E8FF',
        backgroundGradient: 'from-purple-50 to-indigo-100',
        borderStyle: 'border-2 border-purple-200 rounded-xl',
    ...
    offer: {
      discount: '20% OFF',
      bonus: 'Premium Selection',
      code: `SPIR${Date.now().toString().slice(-4)}`,
      validUntil: 'Dec 21, 2025'
    },
    preview: {
  ...
  'premium-grooming': {
    ...
    description: 'High-end grooming services and products',
    icon: '✂️',
    visitorMessage: {
      title: '✂️ Premium Grooming Experience',
      body: 'Experience our premium grooming services. As a visitor, enjoy access to our exclusive grooming products and services.',
      emoji: '✂️',
      theme: {
        primaryColor: '#EC4899',
        secondaryColor: '#FDF2F8',
        backgroundGradient: 'from-pink-50 to-rose-100',
        borderStyle: 'border-2 border-pink-200 rounded-xl',
    ...
    offer: {
      discount: '25% OFF',
      bonus: 'Premium Service',
      code: `GROO${Date.now().toString().slice(-4)}`,
      validUntil: 'Dec 21, 2025'
    },
    preview: {
  ...
  'luxury-spa': {
    ...
    icon: '💆‍♀️',
    visitorMessage: {
      title: '💆‍♀️ Luxury Spa Experience',
      body: 'Relax and rejuvenate at our luxury spa. Visitors enjoy special pricing on our premium wellness treatments.',
      emoji: '💆‍♀️',
      theme: {
        primaryColor: '#06B6D4',
        secondaryColor: '#ECFEFF',
        backgroundGradient: 'from-cyan-50 to-sky-100',
        borderStyle: 'border-2 border-cyan-200 rounded-xl',
    ...
    offer: {
      discount: '35% OFF',
      bonus: 'Luxury Experience',
      code: `SPA${Date.now().toString().slice(-4)}`,
      validUntil: 'Dec 21, 2025'
    },
    preview: {
...
const CampaignManagement: React.FC = () => {
  ...
  // Authentication state - Initialize with token-based defaults
  ...
  const getCurrentShopId = async (): Promise<number> => {
    try {
      ...
      if (!authToken) {
        console.log('⚠️ No auth token, using default shop ID: 6');
        return 6;
      }
      ...
      if (!parsedToken) {
        console.log('⚠️ Failed to parse token, using default shop ID: 6');
        return 6;
      }
  ...
  const applyTemplate = (templateId: CampaignTemplateId) => {
    ...
    setSelectedCampaign((prev: any) => ({
      ...
      visitor_message: template.visitorMessage.body,
      offer_details: `${template.offer.discount} - ${template.offer.bonus}`,
      discount_code: template.offer.code,
      valid_until: template.offer.validUntil
    }));
  };
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {Object.entries(CAMPAIGN_TEMPLATES).map(([key, template]) => (
                    <button
                      key={key}
                      onClick={() => applyTemplate(key as CampaignTemplateId)}
                      className={`p-3 rounded-lg border text-left hover:bg-gray-50 ${template.preview.style}`}
                    >
