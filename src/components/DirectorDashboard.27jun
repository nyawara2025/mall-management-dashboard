  ...
  const fetchDirectorInsights = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMetrics({
          totalCollections: data.total_collections || 4520500,
          outstandingFees: data.outstanding_fees || 1240000,
          studentCount: data.student_count || 342,
          activeStaff: data.staff_count || 28,
          meetings: data.meetings || [
            { id: 1, title: 'Board of Management AGM', date: '28/06/2026', time: '10:00 AM', status: 'Scheduled' },
            { id: 2, title: 'Ministry Procurement Review', date: '02/07/2026', time: '02:30 PM', status: 'Pending' }
          ],
