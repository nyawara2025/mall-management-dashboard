...
export const CropsHub: React.FC<CropsHubProps> = ({ 
  ...
  const [cropsView, setCropsView] = useState<'menu' | 'production' | 'inputs' | 'tasks' | 'weather' | 'market' | 'pathology' | 'soil_analysis' | 'plots_config'>('menu');
  ...
  // 🪵 NEW: Soil Scanning Engine Local States
  const [capturedSoilImage, setCapturedSoilImage] = useState<string | null>(null);
  const [aiAnalyzingSoil, setAiAnalyzingSoil] = useState<boolean>(false);
  ...
  // Form submission handler function for Soil API route communication
  const handleSoilAnalysis = async (base64Image: string) => {
    setAiAnalyzingSoil(true);
    setSoilAnalysisResult(null);
    try {
      const res = await fetch(`https://n8n.tenear.com/webhook/soil-capture`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          image: base64Image,
          environmental_context: {
            ...
            rain_risk: weatherData?.rainProb
          }
        })
      });
      if (res.ok) {
        ...
        alert("Edge Worker pipeline encountered an evaluation error.");
      }
    } catch (e) {
      console.error("Soil analysis routing engine error: ", e);
    } finally {
      setAiAnalyzingSoil(false);
    }
  };

  // 🗺️ ADD THIS NEW NAMED ROUTING FUNCTION RIGHT HERE 👇
  const handleAddNewPlotRouting = () => {
    setCropsView('plots_config');
  ...
  // 🪵 Dedicated Soil Analyzer Screen Panel View with Calibration Layout Overlay
  if (cropsView === 'soil_analysis') {
    return (
      <div className="space-y-4 animate-fadeIn text-left">
        <button 
          onClick={() => {
            const videoEl = document.getElementById('soil-stream-feed') as HTMLVideoElement;
            if (videoEl?.srcObject) {
              (videoEl.srcObject as MediaStream).getTracks().forEach(track => track.stop());
            }
            setCapturedSoilImage(null);
            setSoilAnalysisResult(null);
            setCropsView('menu');
          }} 
          className="text-[11px] text-amber-700 hover:text-amber-800 font-black tracking-wide uppercase"
        >
          ← Cancel & Back to Menu
        </button>
        ...
        <div className="bg-white border border-slate-200/80 p-5 rounded-3xl shadow-xs space-y-4">
          <div>
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-wide">AI Soil Composition Analyzer</h3>
            <p className="text-[10px] text-slate-500 font-bold mt-0.5">Active Target Plot: <span className="text-amber-600">{targetPlot || 'General Field'}</span></p>
          </div>
          ...
          {!capturedSoilImage ? (
            <div className="relative bg-slate-900 rounded-2xl overflow-hidden aspect-video flex items-center justify-center shadow-inner">
              ...
              {/* 🎯 TECHNICAL HUD: Calibration Reference Template Mask Overlay */}
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                {/* Center Capture Target Area */}
                <div className="w-24 h-24 border-2 border-dashed border-amber-400 rounded-full flex items-center justify-center">
                  <div className="w-1.5 h-1.5 bg-amber-400 rounded-full animate-ping" />
                </div>
                
                {/* Horizontal Alignment & Normalization Reference Bars */}
                <div className="absolute bottom-3 left-3 right-3 bg-slate-950/80 backdrop-blur-xs rounded-xl p-1.5 border border-white/10 flex justify-between items-center text-[8px] font-black text-slate-400 uppercase tracking-wider">
                  <div className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 bg-white border border-slate-500 block rounded-xs" />
                    <span>White Ref</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 bg-neutral-900 border border-slate-700 block rounded-xs" />
                    <span>Black Ref</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 bg-amber-800 border border-amber-950 block rounded-xs" />
                    <span>Loam Target</span>
                  </div>
                </div>
              </div>
              ...
              <button
                onClick={() => {
                  const video = document.getElementById('soil-stream-feed') as HTMLVideoElement;
                  if (video) {
                    const canvas = document.createElement('canvas');
                    canvas.width = video.videoWidth || 640;
                    canvas.height = video.videoHeight || 480;
                    ...
                    if (ctx) {
                      ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
                      setCapturedSoilImage(canvas.toDataURL('image/jpeg'));
                      if (video.srcObject) {
                        (video.srcObject as MediaStream).getTracks().forEach(track => track.stop());
                      }
                    }
                ...
                className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-white text-slate-900 font-black text-[10px] tracking-wider uppercase px-5 py-2.5 rounded-xl shadow-xl active:scale-95 transition-all border border-slate-200 pointer-events-auto"
              >
                📸 Capture Ground Matrix
              </button>
            </div>
          ) : (
            <div className="space-y-4">
              <img src={capturedSoilImage} alt="Soil Sample Preview" className="w-full aspect-video object-cover rounded-2xl border border-slate-200 shadow-xs" />
              
              {!soilAnalysisResult && !aiAnalyzingSoil && (
                <div className="flex gap-2">
                  <button 
                    onClick={() => setCapturedSoilImage(null)} 
                    className="w-1/2 border border-slate-200 font-black text-[11px] uppercase tracking-wide py-3 rounded-xl text-slate-600 bg-white hover:bg-slate-50 transition-all"
                  >
                    🔄 Retake Photo
                  </button>
                  <button 
                    onClick={() => handleSoilAnalysis(capturedSoilImage)}
                    className="w-1/2 bg-amber-600 hover:bg-amber-700 text-white font-black text-[11px] uppercase tracking-wide py-3 rounded-xl shadow-xs transition-all"
                  >
                    🪵 Analyze Textures
                  </button>
                </div>
          ...
          {aiAnalyzingSoil && (
            <div className="p-6 text-center space-y-2 border border-slate-100 bg-slate-50/50 rounded-2xl animate-pulse">
              <span className="text-2xl block animate-spin">⏳</span>
              <h5 className="text-xs font-black text-slate-700 uppercase tracking-wider">Evaluating Substrate Imagery...</h5>
              <p className="text-[10px] text-slate-400">Processing color histograms on Cloudflare Edge GPUs.</p>
            </div>
          )}

          {soilAnalysisResult && (
            <div className="bg-amber-50/40 border border-amber-200/60 p-4 rounded-2xl space-y-3 text-[11px] animate-fadeIn">
              <div className="flex justify-between items-center border-b border-amber-200/40 pb-2">
                <div>
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Texture Classification</span>
                  <strong className="text-slate-800 uppercase text-xs">{soilAnalysisResult.soil_type || 'Unknown Type'}</strong>
                </div>
                <span className="px-2 py-0.5 rounded-md font-bold text-[9px] bg-amber-100 text-amber-800">
                  Moisture: {soilAnalysisResult.moisture_estimate || 'Unknown'}
                </span>
              </div>

              <div>
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block mb-1">Agronomic Management Field Advisory</span>
                <p className="text-slate-700 font-medium leading-relaxed">{soilAnalysisResult.advisory}</p>
              </div>
            </div>
          )}
        </div>
  ...
  // ====================================================================
  // 🚀 FIXED GLOBAL PORTAL ZONE
  // This absolute root fallback ensures that whichever modal is active can 
  // overlay on top of ANY view panel seamlessly.
  // ====================================================================

  return (
    <>
      <div className="p-4 text-center border border-dashed rounded-2xl bg-slate-50 text-slate-400 text-xs">
        ⚠️ Fatal Routing Fallback Error: View state context '{cropsView}' is unmapped.
      </div>

      {/* 🗺️ STEP 1: LAND PLOT REGISTRATION MODAL */}
      <PlotConfigModal
        isOpen={isPlotModalOpen}
        onClose={() => setIsPlotModalOpen(false)}
        shopId={shopId}
        ...
        userSession={userSession}
        onPlotSaved={() => {
          fetchActivePlots(); // Instantly fetch newly registered plot coordinates
          setIsPlotModalOpen(false); // Close Plot layout overlay
          setIsCropsModalOpen(true); // 🔥 SUCCESS PIPELINE: Instantly pop up Step 2 to register crops!
        }}
      />

      {/* 🥬 STEP 2: CROP SPECIES SELECTION MODAL */}
