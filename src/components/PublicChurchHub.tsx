    ...
    async function fetchHubData() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // DATA FIX: Dig into the array item to get the services array
    ...
    <div className="min-h-screen bg-gray-50 pb-20 font-sans text-gray-900">
      <header className="bg-white/80 backdrop-blur-md border-b sticky top-0 z-30 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div 
            ...
            className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black shadow-sm cursor-pointer"
          >
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* LEFT: MEMBER PROFILE CARD (RESTORED) */}
          ...
          {/* RIGHT: DASHBOARD OR SERVICE ORDER */}
          <section className="lg:col-span-8 space-y-6">
            {activeView === 'dashboard' ? (
              <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
                ...
                <button 
                  onClick={() => setActiveView('service_order')}
                  className="flex flex-col items-center gap-3 p-8 bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:scale-[1.02] transition-all group col-span-2"
                >
                  ...
                  <span className="font-black text-xs uppercase tracking-widest text-gray-500">Order of Service</span>
                </button>
              </div>
            ) : (
              <div className="space-y-6">
                <button 
                  onClick={() => setActiveView('dashboard')}
                  className="mb-4 text-sm font-bold text-blue-600 flex items-center gap-2 hover:underline"
                >
                   &larr; Back to Dashboard
                </button>
                {services.length > 0 ? (
                  services.map((service, sIdx) => (
                    <div key={sIdx} className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
                      ...
                      <div className="p-8 space-y-10">
                        {service.service_activities?.sort((a,b) => a.sort_order - b.sort_order).map((act, idx) => (
                          <div key={idx} className="relative pl-10 border-l-2 border-blue-50 last:border-0 pb-2">
                            ...
                            <div className="text-gray-800 font-bold leading-relaxed whitespace-pre-line bg-gray-50/50 p-5 rounded-[1.5rem] border border-gray-100 transition-colors">
                              {act.description}
                            </div>
                          </div>
                        ))}
