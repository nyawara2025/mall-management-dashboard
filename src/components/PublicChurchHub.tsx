...
export const ChurchHubLogin = ({ shopId, onLoginSuccess }: { shopId: number, onLoginSuccess: (data: MemberData) => void }) => {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      // Handle empty responses to avoid SyntaxError
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (text) {
            ...
            // Based on your n8n output structure: It's an array with one object containing 'services'
            const extractedServices = Array.isArray(n8nData) ? n8nData[0]?.services : n8nData?.services;
            if (extractedServices) setServices(extractedServices);
          }
        }
      } catch (error) {
          ...
          <section className="lg:col-span-8 space-y-6">
            ...
            {services.length > 0 ? (
              services.map((service, sIdx) => (
                <div key={service.id || sIdx} className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden mb-6">
                  ...
                  <div className="p-8 space-y-10">
                    {service.service_activities?.sort((a,b) => a.sort_order - b.sort_order).map((act, idx) => (
                      <div key={idx} className="relative pl-10 border-l-2 border-blue-50 last:border-0 pb-2">
                        ...
                        <div className="text-gray-800 font-bold leading-relaxed whitespace-pre-line bg-gray-50/50 p-5 rounded-[1.5rem] border border-gray-100">
                          {act.description}
                        </div>
                      </div>
                    ))}
