...
const MeetingsModal = ({ isOpen, onClose, userData }: MeetingsModalProps) => {
  ...
  // 🚀 PLACE THE isLeader FLAG HERE (Right before your API and Event Handlers)
  const isLeader = 
    ...
    userData?.is_region_leader; // 👈 Allows Regional Leaders to pass validation checks
    ...
    try {
      const response = await fetch('https://n8n.tenear.com/webhook/get-church-meetings', {
        ...
        body: JSON.stringify({ 
          ...
          // 🚀 ALIGNED WITH MEMBERS SCHEMA: Sends the user's explicit member_region text field
          region: userData?.member_region 
        }),
      });
      const data = await response.json();
      setMeetings(data);
    ...
    try {
      const response = await fetch('https://n8n.tenear.com/webhook/create-church-meeting', {
        ...
        body: JSON.stringify({
          ...
          region: newMeeting.category === 'Regional' ? userData?.member_region : null 
        }),
      });
      if (response.ok) {
...
export const PublicChurchHub = ({ shopId }: { shopId: number }) => {
  ...
  const LOGO_URL = "https://ufrrlfcxuovxgizxuowh.supabase.co/storage/v1/object/public/church_material/ACKChurchLogo.png";
...
if (isPublicChurchView) {
  ...
  useEffect(() => {
    // 1. First, determine if there is an existing session in storage for the current URL view
    const initialShopId = queryShopId ? Number(queryShopId) : null;
    ...
    let currentShopId = initialShopId;
    let activeUser = null;
    ...
    if (savedAuth === 'true' && savedUser) {
      ...
      try {
        ...
        activeUser = Array.isArray(parsedUser) ? parsedUser[0] : parsedUser;

        setIsAuthenticated(true);
        setUserData(activeUser);

        // Factually update our tracking reference to the member's true tenant ID
        if (activeUser?.shop_id) {
          currentShopId = Number(activeUser.shop_id);
        }
      } catch (e) {
        console.error("Failed to parse initial user storage", e);
      }
    }
    ...
    fetchHubData();
  }, [queryShopId, isAuthenticated, userData?.shop_id]);
  ...
  return (
    <div className="min-h-screen relative font-sans text-gray-900 pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-4">
              ...
              <button
                ...
                className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white rounded-2xl shadow-md transition-all duration-200 transform hover:scale-[1.02] min-h-[140px]"
              >
                <Sparkles className="w-8 h-8 mb-2" />
                <span className="font-bold text-xs md:text-sm tracking-wider uppercase text-center">
                  YOUTH AFFAIRS
                </span>
              </button>
      ...
      <MemberAccountModal 
        ...
        onUpdateSuccess={(updatedUser: any) => {
          // 1. Sync updated state inside the application context memory instantly
          setUserData(updatedUser);
          // 2. Commit updated object dataset down into localStorage cache to persist across browser reloads
          localStorage.setItem(`church_user_${activeShopId}`, JSON.stringify(updatedUser));
        }}
      />
