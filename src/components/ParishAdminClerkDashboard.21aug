    ...
    <div className="min-h-screen bg-slate-100 p-4 sm:p-6 pb-12 text-xs">
      ...
      <header className="bg-white rounded-2xl border border-slate-200 shadow-xs p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-black text-slate-900 uppercase tracking-tight">Parish Clerk Metrics Workspace</h1>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
              {session.name} • <span className="text-blue-700">Data Recording Office</span>
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2 self-end sm:self-auto">
          <span className={`px-2.5 py-1 rounded-full text-[9px] font-black tracking-wide border uppercase ${isOnline ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
            {isOnline ? '● Operational Online' : '○ Offline Storage Mode'}
          </span>
          <button 
          ...
          <button onClick={onLogout} className="bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 font-black tracking-wider px-3 py-2 rounded-xl uppercase flex items-center gap-1.5 transition-colors">
