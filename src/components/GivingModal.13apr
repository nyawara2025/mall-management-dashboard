...
import { X, Heart, Landmark, Gift, HandIcon, MoreHorizontal } from 'lucide-react';
...
export const GivingModal = ({ 
  ...
  const handleGiving = async () => {
    ...
    setSubmitting(true);
    try {
      await fetch('https://n8n.tenear.com/webhook/member-giving', {
        method: 'POST',
      ...
      alert("Thank you for your giving!");
      onClose();
    } catch (error) {
      ...
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative">
        ...
        <div className="space-y-3 mb-8">
          ...
          <div className="relative">
            ...
            <input
              type="number"
              className="w-full pl-12 pr-6 py-5 bg-gray-50 rounded-[2rem] border-none outline-none text-xl font-black"
              placeholder="0.00"
              value={amount}
        ...
        <button 
          ...
          disabled={submitting || !amount || !selectedOption}
          className={`w-full py-5 rounded-3xl text-white font-black text-lg shadow-lg transition-all active:scale-95 ${
            submitting || !amount || !selectedOption ? 'bg-gray-300' : 'bg-blue-600 hover:bg-blue-700'
          }`}
        >
