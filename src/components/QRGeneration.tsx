...
export default function QRGeneration({ preselectedCampaign, onClose }: QRGenerationProps = {}) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (preselectedCampaign) {
      console.log('📱 QRGeneration: Preselected campaign received:', preselectedCampaign);
    ...
    try {
      ...
      for (const visitorType of formData.visitorTypes) {
        for (let i = 0; i < formData.generateCount; i++) {
          ...
          // Path shortened from /qr/checkin (11 chars) to /q (1 char) = 10 char saving
          const path = '/q'; // Ultra-short path
          
          if (formData.qrType === 'claim') {
            // Claim QR - minimal URL for claim-offer workflow
            qrUrl = `${offersBaseUrl}${path}?${qrParams.toString()}`;
          } else {
            // Check-in QR - minimal URL for visitor-checkins workflow  
        ...
        if (i === 0 && visitorType === formData.visitorTypes[0]) {
          ...
          // High-contrast QR with extra margins - this was working before
