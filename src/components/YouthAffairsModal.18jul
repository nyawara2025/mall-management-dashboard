        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1">
          ...
          {activeTab === 'hub' ? (
            ...
            <div className="space-y-4">
              ...
              {errorMessage && (
                <div className="p-2.5 rounded-xl bg-red-50 border border-red-100 text-red-700 text-xs font-medium flex items-center gap-2 animate-fadeIn">
              ...
              {loadingMentors ? (
                ...
                <div className="p-8 text-center bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
                  ...
                  <button 
                    ...
                    className="text-xs font-bold text-purple-600 underline mt-1 block mx-auto hover:text-purple-700"
                  >
                    Clear filters
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 max-h-[380px] overflow-y-auto pr-1">
                  {filteredMentors.map((mentor) => {
                    // Format full name using backend attributes
                    const fallbackName = `${mentor.first_name || ''} ${mentor.last_name || ''}`.trim() || "Church Member";
                    ...
                    return (
                      ...
                      >
                        {/* Avatar Wrapper using extracted string tokens */}
                        ...
                        <div className="flex-1 space-y-1">
                          ...
                          <p className="text-xs text-gray-500 leading-relaxed">
                            {mentor.bio || `Available to mentor youth in the field of ${mentor.profession || 'Spiritual Growth'}.`}
                          </p>

                          {/* Direct Connection and Middleware Relayer Action Trigger */}
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                // Optimistically flag this card row item as requested in local layout
                                setMentors(prev => prev.map(m => m.id === mentor.id ? { ...m, is_requested: true } : m));
                              } catch (err) {
                                setErrorMessage('Could not relay connection request via app chat channels.');
                              } finally {
                            ...
                            className={`w-full mt-2 py-1.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 border transition-all ${
                              mentor.is_requested 
                                ? 'bg-emerald-50 border-emerald-200 text-emerald-700 cursor-default shadow-none'
                                : 'bg-purple-600 border-purple-600 hover:bg-purple-700 text-white shadow-sm active:scale-[0.98]'
                            }`}
                          >
                            {requestingMentorId === mentor.id ? (
                              ...
                              <>
                                <CheckCircle2 className="w-3.5 h-3.5" /> Direct Message Dispatched
                              </>
                            ) : (
                              'Request Connection via Chat'
                            )}
                          </button>
                        </div>
                      </div>
            ...
            <div className="space-y-4">
              {selectedThread ? (
                ...
                <div className="space-y-4 animate-fade-in">
                  {/* Back Navigation Bar inside individual topics */}
                  <button 
                    type="button" 
                  ...
                  {/* Main Original Parent Post Topic Card Header */}
                  ...
                  {/* Dynamic Scroll Container Output for Active Comments Stream */}
                ...
                <>
                  <div className="flex flex-col gap-2">
                    {/* Scope Switcher Controls - Driven securely by parent-inherited flags */}
                  ...
                  {/* Fast Inline Post Composer Node */}
