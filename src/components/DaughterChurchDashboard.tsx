...
export const DaughterChurchDashboard: React.FC<DaughterChurchDashboardProps> = ({ session, onLogout }) => {
  ...
  const [churchName, setChurchName] = useState<string>('LOCAL ASSEMBLY HUB'); // 👈 Add this line for dynamic renaming
  ...
  const [period, setPeriod] = useState(session.reporting_period || '2026-W31');
  const [attendance, setAttendance] = useState('');
  const [sacraments, setSacraments] = useState('');

  // 🟢 State variables for granular tracking
  const [attMale, setAttMale] = useState<number>(0);
  const [attFemale, setAttFemale] = useState<number>(0);
  const [attYouth, setAttYouth] = useState<number>(0);
  const [attChildren, setAttChildren] = useState<number>(0);
  const [visitorsCount, setVisitorsCount] = useState<number>(0);
  const [communicantsCount, setCommunicantsCount] = useState<number>(0);

  // Granular Collection Inputs for Remittance Quota calculations
  const [titheCollection, setTitheCollection] = useState<string>('0');
  const [offertoryCollection, setOffertoryCollection] = useState<string>('0');
  const [thanksgivingCollection, setThanksgivingCollection] = useState<string>('0');
  const [ministryGroupCollection, setMinistryGroupCollection] = useState<string>('0');
  ...
  const fetchLocalChurchData = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        // 👈 Extract and set the real church name from the response object
        if (data.church_info && data.church_info.name) {
          setChurchName(data.church_info.name);
        } else if (data.church_name) {
  ...
  const handleSubmitKPI = async (e: React.FormEvent) => {
    ...
    // 1. Pack variables into an audit-ready, structured payload
    const payload = {
      tenant_id: session.assigned_id,
      reporting_period: period, // e.g., "2026-W31"
    
      // Granular Demographic Breakdown (Section 7, Module 3)
      demographics: {
        adult_male: attMale,
        adult_female: attFemale,
        youth: attYouth,
        children: attChildren
      },
    
      // Worship Split Metrics
      sacraments_administered: parseInt(sacraments, 10) || 0,
      communicants_count: communicantsCount,
      visitors_count: visitorsCount,
    
      // Dynamic dynamic cross-cohort calculation for aggregate historical totals
      worship_attendance: attMale + attFemale + attYouth + attChildren,

      // Core Collections Mapping for Remittance Quota calculations
      collections: {
        tithe: parseFloat(titheCollection) || 0,
        offertory: parseFloat(offertoryCollection) || 0,
        thanksgiving: parseFloat(thanksgivingCollection) || 0,
        ministry_group: parseFloat(ministryGroupCollection) || 0
      },

      // Tier 5 Setup: Submits as an unverified draft to the Mother Parish (Section 4 Rule)
    ...
    try {
      const res = await fetch('https://n8n.tenear.com/webhook/ack-submit-kpi', {
        ...
        headers: { 
          ...
          'x-audit-reason': 'Granular weekly congregational metrics submission' // Audit trail hook 
        },
      ...
      if (res.ok) {
        ...
        resetKPIFormFields(); // Step 3 clean-up hook
        fetchLocalChurchData();
      } else {
        alert("Server Error: Your n8n gateway node rejected the registration payload.");
      }
    } catch (err) {
  ...
  const resetKPIFormFields = () => {
    ...
    setAttChildren(0);
    setVisitorsCount(0);
    setCommunicantsCount(0);
    setTitheCollection('0');
    setOffertoryCollection('0');
    setThanksgivingCollection('0');
    setMinistryGroupCollection('0');
  };
  ...
  // 📸 MVP No. 12: Refactored Zero-Dependency n8n Direct Upload Handler
  const handleProjectPhotoUpload = async (projectId: number, files: FileList | null) => {
    if (!files || files.length === 0) return;
    const file = files[0];

    // Enforce file size restrictions to protect mobile cellular bandwidth channels
    if (file.size > 3 * 1024 * 1024) {
      alert("Validation Error: Photograph file size cannot exceed 3MB to optimize mobile network uploads.");
      return;
    }

    setUploadingProjectId(projectId);

    // 1. Initialize a clean FileReader module to read the image binary locally
    const reader = new FileReader();
    reader.readAsDataURL(file);
    
    reader.onload = async () => {
      const base64String = reader.result as string; // Raw base64 text stream array data

      try {
        // 2. Post the text payload data string directly to your n8n API gateway node
        const res = await fetch('https://n8n.tenear.com/webhook/update-project-photos', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...
            tenant_id: session.assigned_id,
            file_name: file.name,
            image_base64: base64String // Streamed straight into n8n memory nodes
          })
        });

        if (res.ok) {
          alert("Project photograph evidence committed and pinned to blueprint records successfully!");
          fetchLocalChurchData(); // Sync live view list arrays
        } else {
          alert("Server Error: Your n8n gateway node rejected the image processing payload.");
        }
      } catch (err) {
        console.error("n8n photo upload exception trace:", err);
        alert("Upload Exception: Unable to establish connection to n8n intake gateways.");
      } finally {
    ...
    reader.onerror = (error) => {
      console.error("FileReader exception trace:", error);
      setUploadingProjectId(null);
    };
  };
      ...
      <header className="bg-white rounded-2xl border border-slate-200 shadow-xs p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-sm font-black text-slate-900 uppercase tracking-tight truncate">{churchName}</h1>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
            ...
            <form onSubmit={handleSubmitKPI} className="space-y-4">
              ...
              {/* 👥 GRANULAR DEMOGRAPHIC COHORTS (Section 7, Module 3) */}
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1">
                  ...
                  <input
                    ...
                    className="bg-transparent w-full text-xs font-bold text-slate-700 focus:outline-none"
                    value={attMale || ''}
                    onChange={e => setAttMale(parseInt(e.target.value) || 0)}
                  />
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1">
                  <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wide">Adult Women</label>
                  <input
                    type="number"
                    placeholder="0"
                    required
                    className="bg-transparent w-full text-xs font-bold text-slate-700 focus:outline-none"
                    value={attFemale || ''}
                    onChange={e => setAttFemale(parseInt(e.target.value) || 0)}
                  />
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1">
                  ...
                  <input
                    ...
                    className="bg-transparent w-full text-xs font-bold text-slate-700 focus:outline-none"
                    value={attYouth || ''}
                    onChange={e => setAttYouth(parseInt(e.target.value) || 0)}
                  />
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1">
                  <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wide">Sunday School Children</label>
                  <input
                    type="number"
                    placeholder="0"
                    required
                    className="bg-transparent w-full text-xs font-bold text-slate-700 focus:outline-none"
                    value={attChildren || ''}
                    onChange={e => setAttChildren(parseInt(e.target.value) || 0)}
                  />
                </div>
              </div>

              {/* 📋 WORSHIP ANALYSIS LINES (Communicants & Visitors Split) */}
              <div className="grid grid-cols-2 gap-3 border-t border-slate-100 pt-3">
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1">
                  <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wide">Communicants</label>
                  <input
                    type="number"
                    placeholder="0"
                    required
                    className="bg-transparent w-full text-xs font-bold text-slate-700 focus:outline-none"
                    value={communicantsCount || ''}
                    onChange={e => setCommunicantsCount(parseInt(e.target.value) || 0)}
                  />
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1">
                  <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wide">Visitors</label>
                  <input
                    type="number"
                    placeholder="0"
                    required
                    className="bg-transparent w-full text-xs font-bold text-slate-700 focus:outline-none"
                    value={visitorsCount || ''}
                    onChange={e => setVisitorsCount(parseInt(e.target.value) || 0)}
                  />
                </div>
              </div>  
                    ...
                    <tr key={idx} className="hover:bg-white transition-colors">
                      ...
                      <td className="p-3 text-center">
                        <div className="flex flex-col items-center gap-1.5">
                          ...
                          {/* Conditional Rendering: Photo Thumbnail or Upload Button Trigger */}
                          {project.photograph_evidence_url ? (
                            <a 
                              href={project.photograph_evidence_url} 
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="block w-9 h-9 rounded-xl overflow-hidden border border-slate-200 shadow-xs hover:opacity-80 transition-opacity bg-slate-50 shrink-0"
                            >
                              <img 
                                src={project.photograph_evidence_url} 
                                alt="Evidence" 
                            ...
                            <div className="relative">
                              <input
                                ...
                                onChange={(e) => handleProjectPhotoUpload(project.id, e.target.files)}
                              />
                              <label
                                htmlFor={`upload-project-${project.id}`}
                                className={`cursor-pointer bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 font-black text-[8px] tracking-wide px-2 py-1 rounded-lg uppercase inline-block transition-colors select-none ${
                                  uploadingProjectId === project.id ? 'opacity-50 cursor-not-allowed' : ''
                                }`}
                              >
                                {uploadingProjectId === project.id ? 'Saving...' : '📸 Upload'}
                              </label>
                            </div>
                          )}

                          {/* Progress Bar & Percentage Indicators stacked neatly underneath */}
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="font-mono text-[9px] font-bold text-slate-500">{project.percentage_progress}%</span>
                          </div>
