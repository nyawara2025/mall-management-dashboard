  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm print:p-0 print:bg-white">
      <div className="w-full max-w-6xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] print:max-h-full print:shadow-none print:rounded-none">
        ...
        {/* 3. Central Ledger Data Registry Canvas Table */}
        <div className="flex-1 overflow-auto p-6 bg-white print:p-0">
          {loading ? (
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white print:border-none print:shadow-none">
              <table className="w-full text-left border-collapse">
                <thead>
                 <tr className="bg-gray-50 border-b border-gray-200 text-[10px] font-bold text-gray-600 uppercase tracking-wider print:bg-gray-100">
                    <th className="p-3.5">Member Phone</th>
                    <th className="p-3.5">{activeTab === 'pillars' ? 'Strategic Pillar Category' : 'Community Initiative Category'}</th>
                    <th className="p-3.5">Indicator Performance Matrix</th>
                    <th className="p-3.5">Impact Analysis & Evidence</th>
                    <th className="p-3.5">Evidence Attachments</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 text-xs text-gray-700">
                  {reportData.map((record, index) => (
                    <tr key={index} className="hover:bg-gray-50/40 transition-colors print:break-inside-avoid">
                      <td className="p-3.5 font-semibold text-gray-900 whitespace-nowrap">{record.member_phone}</td>
                      <td className="p-3.5 font-bold text-blue-900 max-w-[200px]">{activeTab === 'pillars' ? record.pillar_name : record.programme_name}</td>
                      <td className="p-3.5 space-y-1.5 min-w-[220px]">
                        {record.indicators?.map((ind: any, i: number) => (
                          <div key={i} className="bg-gray-50 p-1.5 rounded border border-gray-100/80">
                            <span className="block text-[10px] uppercase font-bold text-gray-500">{ind.label}</span>
                            <span className="font-semibold text-gray-800">{ind.value || <span className="text-gray-300 italic">Not logged</span>}</span>
                          </div>
                        ))}
                      </td>
                      <td className="p-3.5 text-gray-600 font-medium leading-relaxed max-w-[260px] break-words">
                        ...
                        {record.file_urls && record.file_urls.length > 0 ? (
                          record.file_urls.map((url: string, uIdx: number) => (
                            <a 
                              ...
                              target="_blank" 
                              rel="noreferrer" 
                              className="inline-flex items-center gap-1 text-[11px] text-blue-600 hover:underline font-bold bg-blue-50 px-2 py-0.5 rounded border border-blue-100 block print:hidden"
                            >
                              Acknowledge Link
                            </a>
                          ))
                        ) : (
                          <span className="text-gray-300 italic">No file links</span>
                        )}
                        {record.file_urls?.map((url: string, uIdx: number) => (
