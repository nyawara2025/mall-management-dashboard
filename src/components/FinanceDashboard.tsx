...
export const FinanceDashboard = () => {
  // Destructure tenant shop identifier alongside regular auth metadata
  const { user } = useAuth();
  ...
  // 1. Fetch via POST Webhook Handler
  useEffect(() => {
    const fetchTithesFromWebhook = async () => {
      try {
        setIsLoadingTithes(true);
        
        // Strict POST payload with tenant identification context
        ...
        if (!response.ok) throw new Error('Network error syncing ledger via webhook');
        
        const result = await response.json();
        setTitheEntries(result.data || []);
      } catch (err) {
        console.error('Error syncing church_tithes ledger payload:', err);
      } finally {
    ...
    if (user) {
      fetchTithesFromWebhook();
    }
  }, [user, shopId]);

  // 2. Mutation / Update via POST Webhook Handler
  const handleUpdateManualTithe = async (id: string) => {
    try {
      ...
      // Strict POST mutation request matching pattern criteria
      const response = await fetch('https://n8n.teneear.com/webhook/update-manual-tithe', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) throw new Error('Failed processing ledger transaction mutation');
    } catch (err) {
      console.error('Failed processing ledger mutation webhook:', err);
      // Optional: Trigger re-fetch loop here to restore database state matching upon execution fail
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-6 md:p-8 font-sans">
      ...
      {/* 2. CHURCH BRANDING SUB-HEADER */}
      <div className="max-w-7xl mx-auto flex justify-between items-start mb-8">
        <div>
          <h2 className="text-2xl font-serif italic text-slate-700 tracking-wide">Treasury & Finance</h2>
          <p className="text-sm font-semibold text-blue-600 mt-0.5">
            St. Barnabas Anglican Church, Otiende - Langata <span className="font-normal italic text-gray-400 ml-1">- The Home of Encouragement!</span>
          </p>
        </div>
        <div className="w-16 h-16 opacity-90 hover:opacity-100 transition-opacity">
          <img src="/church-crest.png" alt="Church Crest" className="w-full h-full object-contain" />
        </div>
      </div>
      ...
      {/* 5. TITHE VERIFICATION CARD (POWERED VIA POST WEBHOOKS) */}
                  ...
                  {titheEntries.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className={`hover:bg-slate-50/50 transition-colors ${isMpesa ? 'bg-emerald-50/5' : ''}`}>
                        {/* Timestamp formatting for Kenya Time (en-KE) */}
                        ...
                        {/* Contributor identification slot */}
                        <td className="py-4 px-4 text-slate-900 font-semibold">
                          {isEditing ? (
                            <input
                              type="text"
                              className="border border-slate-200 rounded-xl px-3 py-1.5 text-xs w-full focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white"
                              value={editForm.contributor_name}
                              onChange={e => setEditForm({ ...editForm, contributor_name: e.target.value })}
                            />
                          ) : (
                            <div className="flex items-center space-x-2">
                              <span>{item.contributor_name || 'Anonymous Giver'}</span>
                              {isMpesa && <span className="text-[10px] text-slate-400 font-mono font-normal">[{item.mpesa_receipt_number || 'SYSTEM'}]</span>}
                            </div>
                          )}
                        </td>

                        {/* Transaction channel badge */}
                        ...
                        {/* Auditable Currency output fields */}
                        <td className="py-4 px-4 text-right font-bold text-slate-900 text-sm whitespace-nowrap">
                          {isEditing ? (
                            <input
                              type="number"
                              className="border border-slate-200 rounded-xl px-3 py-1.5 text-xs w-32 text-right focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white"
                        ...
                        {/* Visual immutability guards indicating platform verification status */}
                        ...
                        {/* Functional mutations controls based on verification rules */}
                        <td className="py-4 px-4 text-right whitespace-nowrap">
                          {isMpesa ? (
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1 text-[11px] font-bold text-blue-600 hover:bg-blue-50 px-3 py-1 rounded-xl transition-all border border-transparent hover:border-blue-100/50"
                            >
