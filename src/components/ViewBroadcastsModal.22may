...
export const ViewBroadcastsModal = ({ isOpen, onClose, userData }: any) => {
  ...
  const handleCreateBroadcast = async () => {
    ...
    setSending(true);
    try {
      // 1. Placeholder for handling raw base64 or file uploads if a file is attached
      let currentFileUrl = '';
      if (selectedFile) {
        // You can attach your file handler here (e.g. upload to Supabase storage or pass as base64)
        currentFileUrl = `uploaded_media_${Date.now()}_${selectedFile.name}`;
      }

      // 2. Transmit fully loaded dynamic payload out to n8n
      const response = await fetch('https://n8n.tenear.com/webhook/create-church-broadcast', {
        ...
        body: JSON.stringify({
          ...
          target_type: newBroadcast.target_scope,
          target_name: newBroadcast.target_name,
          attachment_path: currentFileUrl,
          // Forwarding exact lookup context IDs
          target_id: newBroadcast.target_scope === 'zone' ? userData?.zone_id : userData?.ministry_id
        }),
      });
      ...
      if (response.ok) {
        ...
        setIsCreating(false);
        fetchMessages(); // Auto-refresh updates feed list
      } else {
