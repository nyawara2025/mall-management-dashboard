...
export const PublicSchoolHub = ({ shopId, user: initialUser }: { shopId: number; user?: any }) => {
  ...
  const [activePortalView, setActivePortalView] = useState<'grid' | 'homework' | 'fees' | 'bulletins' | 'teacher-dashboard' | 'driver-portal' | 'transport'>('grid');     
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-10 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        {activePortalView === 'grid' && (
          <div className="max-w-md mx-auto space-y-6 text-center animate-in fade-in duration-200">
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Card 5: Transport Active Dashboard Tracker */}
              <button 
                type="button"
                onClick={() => setActivePortalView('transport')}
              ...
              >
                <div className="p-4 bg-white rounded-2xl text-amber-600 shadow-sm group-hover:scale-105 transition-transform">
                  <Bus size={24} />
                </div>
                <span className="text-[11px] font-black text-slate-700 uppercase tracking-wide leading-tight">
                  Transport &<br/>Shuttles
                </span>
              </button>
              ...
              data.homework.map((item: any, idx: number) => (
                <div key={idx} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                  ...
                  <div className="flex justify-between items-center border-b border-slate-50 pb-3">
                    <span className="text-[9px] font-black bg-blue-50 text-blue-600 px-2.5 py-1 rounded-md border border-blue-100/50 uppercase tracking-wider">
                      {item.subject}
                    </span>
                  ...
                  <div className="grid grid-cols-2 gap-3 bg-slate-50/50 p-4 rounded-2xl border border-slate-100/60 text-xs">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-slate-700 mt-0.5">Pg. {item.pages || 'N/A'}</p>
                    </div>
                    <div>
                      <span className="text-[9px] font-black text-slate-400 uppercase block tracking-wider">Assigned Exercise</span>
                      <p className="font-bold text-slate-700 mt-0.5">{item.exercise || 'N/A'}</p>
                    </div>
                    <div>
                      <span className="text-[9px] font-black text-slate-400 uppercase block tracking-wider">Task Question Scope</span>
                      <p className="font-bold text-blue-600 mt-0.5">
                        {item.question_from && item.question_to 
                          ? `Qno. ${item.question_from} to ${item.question_to}` 
                          : 'Complete Scope'}
                      </p>
                    </div>
                  </div>
        ...
        {/* VIEW 5: SUB PANEL - LIVE SCHOOL BUS GPS MAP TRACKER */}
        {activePortalView === 'transport' && (
          <div className="max-w-2xl mx-auto space-y-4 text-left animate-in fade-in duration-200">
            <div className="mb-2">
              <button 
                type="button" 
            ...
            {/* Verification isolation gate: Confirms data payload context has a linked vehicle route */}
            {data?.transport?.assigned_route_id ? (
              <div className="space-y-4">
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                  <span className="text-[9px] font-black bg-amber-50 text-amber-600 px-2.5 py-1 rounded-md border border-amber-100 uppercase">
                    Assigned Shuttle Profile
                  </span>
                  <h4 className="font-black text-slate-800 text-base mt-3 mb-1.5">
                    {data.transport.route_name || "School Bus Route"}
                  </h4>
                  <div className="text-xs text-slate-500 font-bold space-y-1">
                    <p>Driver Operator: {data.transport.driver_name || "Assigned Driver"}</p>
                    <p>Contact Reference: {data.transport.driver_phone || "Not Listed"}</p>
                  </div>
                </div>
              ...
              <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm text-center py-12">
                ...
                <p className="text-slate-700 font-black text-sm uppercase tracking-wide">No Shuttle Assigned</p>
                <p className="text-slate-400 text-xs mt-1 max-w-xs mx-auto font-medium">
                  There are no active bus transport route configurations mapped to this student profile record at the moment.
                </p>
              </div>
            )}
          </div>
