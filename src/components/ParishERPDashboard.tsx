  ...
  const handleProvisionStaff = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://n8n.tenear.com/webhook/ack-provision-staff', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          admin_user_id: session.assigned_id, // For the Audit Trail Actor Link
        ...
        <div className="lg:col-span-3 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs space-y-4">
          <div className="border-b border-slate-100 pb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium">Delegated data recording permissions managed under Parish authority</p>
            </div>
            
            {/* 🔒 Secondary safety gate: Only allow management roles to provision new accounts */}
