...
import { ShieldAlert, BarChart3, AlertCircle, CheckCircle, Loader2, Send } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';

export function AgentDashboard() {
  const { user } = useAuth(); // Sourced directly from your platform context (contains shop_id and phone)
  ...
  const handleAgentSubmission = (actionType: 'incident' | 'tally' | 'other', formData: Record<string, any>) => {
    ...
    const basePayload = {
      action: actionType,
      shop_id: user?.shop_id || 65, 
      voter_phone: (user as any)?.phone || (user as any)?.voter_phone || "0716300197",
  ...
  const transmitToN8n = async (finalData: any) => {
    try {
      // Maps straight to your open, armed n8n webhook processing node endpoint
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col max-w-md mx-auto border-x border-gray-100 shadow-xl font-sans">
      {/* Mobile Top App Bar */}
      <div className="bg-gray-900 text-white p-6 rounded-b-[2rem] shadow-md">
        <span className="text-[10px] bg-blue-600 text-white font-black px-2.5 py-1 rounded-full uppercase tracking-wider">Field Operations</span>
        <h2 className="text-xl font-black mt-2 text-white">2027 Agent Command Portal</h2>
        <p className="text-xs text-gray-400 font-bold mt-1">
          Agent Phone: {(user as any)?.phone || (user as any)?.voter_phone || '0716300197'}
        </p>
      </div>
          ...
          <div className="space-y-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm animate-in slide-in-from-bottom duration-200">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button 
                ...
                onClick={() => handleAgentSubmission('tally', { polling_station_name: stationName, reach_out: parseInt(candidateVotes, 10), sentiment_score: parseFloat(competitorVotes) })}
              ...
              >
                {loading ? <Loader2 className="animate-spin" size={18} /> : <><Send size={16} /><span>Submit Tally</span></>}
              </button>
            </div>
          </div>
        )}
