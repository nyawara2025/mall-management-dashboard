...
export function AgentDashboard() {
  ...
  // 🔐 INTERCEPT FORM LAYER: Render if component session is absent
  if (!agentUser) {
    if (checkingSession) {
      return (
  ...
  const handleCustomWebhookAuth = async (e: React.FormEvent) => {
      ...
      try {
        ...
        // Secure baseline storage persistence bound to this specific campaign node context
  ...
  const handleAgentSubmission = (actionType: 'incident' | 'tally' | 'other', formData: Record<string, any>) => {
    ...
    // FIXED: Changed old 'user' properties to leverage your secure 'agentUser' token properties
    const basePayload = {
      action: actionType,
