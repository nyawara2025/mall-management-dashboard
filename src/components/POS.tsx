...
import { ShoppingCart, Search, Trash2, X, Smartphone, Banknote, Settings, ArrowLeft, Image as ImageIcon } from 'lucide-react';
...
import { StaffSelection } from './StaffSelection.tsx';
...
export const POS: React.FC<POSProps> = ({ shopId }) => {
  ...
  const [waiterName, setWaiterName] = useState<string>(''); // Add this

  const [activeTab, setActiveTab] = useState<'pos' | 'staff'>('pos');

  const MASTER_ADMIN_PIN = "9999";

  const handleOpenAdmin = () => {
    const enteredPin = prompt("Enter Administrator Master PIN to unlock settings:");
  
    if (enteredPin === MASTER_ADMIN_PIN) {
      setActiveTab('staff');
    } else if (enteredPin !== null) {
      alert("⚠️ Restricted Access: Unauthorized Attempt.");
    }
  };
  ...
  const executeFinalSale = async () => {
    ...
    // Guard clause: Ensure waiter is selected
    if (!waiterName) {
    ...
    try {
      ...
      console.warn("Network issue detected, saving locally...");
       ...
       try {
         // MAP CART ITEMS TO INCLUDE final_price EXPLICITLY
         const formattedCart = cart.map(item => ({
           ...item,
           // Ensure final_price is a number and exists for the Postgres node
           final_price: Number(item.final_price) || Number(item.base_price) || 0
         }));

         await saveOfflineSale({
           cart: formattedCart, // Use the formatted cart here
           total: calculateTotal(),
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      // --- NEW: LOAD FROM INDEXEDDB FIRST ---
      try {
        const cachedProducts = await getLocalProducts();
        if (cachedProducts.length > 0) {
          // Type cast safely to your component's Product interface
          setProducts(cachedProducts as unknown as Product[]);
        }
      } catch (dbErr) {
        console.warn('Could not read from local DB:', dbErr);
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          // --- NEW: SAVE TO INDEXEDDB FOR OFFLINE USE ---
          // Mapping to match the DB interface keys if necessary
          const toCache = grouped.map(p => ({ ...p, id: p.product_id, shop_id: shopId.toString() }));
          await saveProductsLocally(toCache as any);
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen gap-4 p-4 bg-gray-50 text-gray-800 relative">
      ...
      {activeTab === 'staff' ? (
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm border p-6 overflow-y-auto flex flex-col">
          <button 
            onClick={() => setActiveTab('pos')}
            className="mb-6 w-fit flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-xs font-black uppercase hover:bg-blue-600 hover:text-white transition-all"
          >
        ...
        <>
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-sm border p-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between mb-4 gap-4">
              ...
              <button 
                onClick={handleOpenAdmin} // Use the handler here
                className="p-2.5 bg-gray-100 text-gray-400 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all border border-transparent hover:border-blue-200"
          ...
          <div className="w-full md:w-96 bg-white rounded-2xl shadow-sm border p-4 flex flex-col">
            ...
            <StaffSelection 
              ...
              onSelectWaiter={setWaiterName} 
            />

            <h2 className="text-lg font-black my-4 flex items-center gap-2 border-b pb-4 uppercase tracking-tight">
