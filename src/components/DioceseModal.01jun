...
// ==========================================
// Isolated Form Component Using Strict n8n POST Routes
// ==========================================
const InteractivePillarForm = ({ 
  ...
  // 1. Load active database parameters via n8n query on row accordion open
  useEffect(() => {
    const fetchRowData = async () => {
      if (!userId || !dioceseId) return;
      try {
        ...
        console.error('Failed fetching data via n8n hook:', err);
      }
    };
    fetchRowData();
  }, [title, userId, dioceseId]);

  // 2. Transmit multipart file binaries to n8n upload workflows
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append('dioceseId', dioceseId);
  ...
  // 3. Commit text nodes and file collections to database records via n8n
  const handleSaveData = async () => {
    if (!userId || !dioceseId) {
      setMessage({ type: 'error', text: 'Session context missing. Please log in.' });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Workspace successfully saved!' });
    } catch (err: any) {
  ...
  return (
    <div className="mt-3 p-4 bg-gray-50 rounded-xl border border-gray-100 space-y-4">
      {/* Activity Input Field Area */}
      <div>
        <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">
          Activity Updates / Content Notes
        </label>
        <textarea
          rows={3}
          value={content}
          onChange={(e) => setContent(e.target.value)}
          placeholder="Type active milestones, operational requests, or feedback notes for this group..."
          className="w-full p-3 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all"
        />
      </div>
      ...
      <div>
        <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">
          Supporting Files & Reports
        </label>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="flex items-center justify-center gap-2 px-4 py-3 bg-white border border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
            {isUploading ? (
            ...
            {fileUrls.length === 0 ? (
              ...
              fileUrls.map((url, index) => (
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer" 
                  className="inline-flex items-center gap-1 text-xs text-blue-600 hover:underline font-medium truncate max-w-[280px]"
                >
                  <FileText className="w-3 h-3 flex-shrink-0" /> Attachment #{index + 1}
                </a>
              ))
            )}
          </div>
      ...
      {/* Processing Status Action Row Bar */}
      <div className="flex items-center justify-between pt-2 border-t border-gray-200">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-semibold shadow-sm transition-all disabled:opacity-40"
        >
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 bg-white text-sm text-gray-600 border-t border-gray-50 pt-3">
                    ...
                    {userId ? (
                      ...
                      <div className="mt-2 p-3 bg-amber-50 rounded-lg text-amber-700 text-xs font-medium border border-amber-100">
                        Please sign in to update content reports or upload supporting media files.
                      </div>
                    )}
                  </div>
                )}
