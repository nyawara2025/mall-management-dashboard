...
export const ArchdeaconryDashboard: React.FC<any> = ({ session, onLogout }) => {
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <header className="bg-white p-4 rounded-xl border flex justify-between items-center shadow-xs">
        <div className="flex items-center gap-3">
          <Globe className="w-5 h-5 text-blue-700" />
          <h2 className="text-sm font-black uppercase tracking-tight">Archdeaconry Regional Hub: {session.name}</h2>
        </div>
      ...
      <main className="mt-6 text-center text-xs text-slate-400 font-medium py-12 border border-dashed rounded-xl bg-white">
        Archdeaconry Regional M&E Metrics Loading...
      </main>
    </div>
  );
};
